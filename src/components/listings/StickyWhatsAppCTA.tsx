...
/**
 * CTA sticky inferior — botón naranja "Contactar por WhatsApp" + favorito.
...
export function StickyWhatsAppCTA({ listing, schoolName }: Props) {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-30 mx-auto max-w-2xl border-t border-border/70 bg-background/95 px-5 pt-3 pb-safe backdrop-blur-xl">
      <div className="flex items-center gap-2.5 pb-3">
        {isOwner ? (
          <Link
            href="/mis-publicaciones"
            className="flex h-14 w-full items-center justify-center rounded-full bg-foreground text-[0.95rem] font-semibold text-background transition-transform active:scale-[0.98]"
          >
          ...
          <>
            ...
            >
              <span
                aria-hidden
                className="absolute inset-0 bg-gradient-to-b from-white/15 to-transparent opacity-80"
              />
              <WhatsAppGlyph className="relative size-[1.15rem]" />
